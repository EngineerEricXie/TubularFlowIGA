#!/usr/bin/env python3
"""Check distributed transient graph transactions, clocks, and serial port/field parity."""
import argparse
import json
import math
import os
...
def main():
    ...
    binary = repo/'solvers/cpu/immersed_transient_distributed_domain_test'
    ...
    try:
        ...
        for ranks, split in cases:
            ...
            if result.returncode:
                raise RuntimeError(f'domain/{ranks}: nonzero launcher exit')
            for rank in range(ranks):
                ...
                if report['returncode'] or report['timed_out'] or not report['resource']:
                    raise RuntimeError('failed or unmeasured domain rank')
                case['rank_reports'].append(report)
                lines = [line for line in (rd/'stdout.log').read_text().splitlines()
                         if line.startswith('immersed_transient_domain_mpi ')]
                if len(lines) != 1 or not lines[0].endswith(' passed'):
                    raise RuntimeError('missing domain MPI completion observation')
                ...
                if (any(not math.isfinite(value) for value in values.values())
                        ...
                        or values['steps'] != 2 or values['cells'] != 4 or values['owned_rows'] < 0):
                    raise RuntimeError('invalid domain MPI numerical/solve observations')
                case['observations'].append(values)
            case['status'] = 'passed'
            print('domain', directory.name, 'passed', flush=True)
        if digest(binary) != summary['binary_sha256']:
            raise RuntimeError('domain MPI binary changed during validation')
