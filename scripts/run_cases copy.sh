...
Prepare3DCase()
{
	...
	local base_work_dir=$run_root/${example//\//_}
	local work_dir
	local prepare_ranks=$ranks

	work_dir=$(NextAvailablePath "$base_work_dir")
	...
	mkdir -p "$work_dir"
	if [[ $work_dir != "$base_work_dir" ]]; then
		printf 'note    output directory exists; using %s\n' "$work_dir"
	fi
...
RunOneDCase()
{
	local example=$1
	local case_name=${example##*/}
	local base_output_dir=$run_root/${example//\//_}_output
	local output_dir

	output_dir=$(NextAvailablePath "$base_output_dir")
