#!/usr/bin/env python3
"""Check distributed transient Newton, committed history, and timestep transactions."""
import argparse
import json
import math
import os
...
def main():
    ...
    parser.add_argument('--modes', nargs='+', choices=['flow','pressure','closed','inertial','empty-work'], default=['flow','pressure','closed','inertial','empty-work'])
    ...
    binary = repo/'solvers/cpu/immersed_transient_distributed_runtime_test'
    summary = dict(status='running', weighted=args.weighted, binary=str(binary), binary_sha256=digest(binary), cases=[])
    ...
    try:
        ...
        for mode, ranks, split in cases:
            ...
            directory.mkdir()
            command = ['timeout', '--kill-after=5s', '450s', 'mpiexec', '--oversubscribe', '-np', str(ranks),
                       sys.executable, str(repo/'scripts/hpc_rank_run.py'), '--expected-ranks', str(ranks),
                       '--timeout', '420', '--output-dir', str(directory), '--', str(binary), mode]
            if split:
            ...
            for rank in range(ranks):
                ...
                if report['returncode'] or report['timed_out'] or not report['resource']:
                    raise RuntimeError('failed or unmeasured transient runtime rank')
                case['rank_reports'].append(report)
                lines = [line for line in (rd/'stdout.log').read_text().splitlines()
                         if line.startswith('immersed_transient_runtime_mpi ')]
                if len(lines) != 1 or not lines[0].endswith(' passed'):
                    raise RuntimeError('missing transient runtime MPI completion observation')
                ...
                if (any(not math.isfinite(value) for value in values.values())
                        ...
                        or not 0 <= values['physical_fraction'] <= 1
                        or values['history_difference'] <= 1e-9
                        or values['owned_rows'] < 0 or values['owned_history_nodes'] < 0
                        or values['assembly_s'] <= 0 or values['solve_s'] <= 0):
                    raise RuntimeError('invalid transient runtime MPI numerical/solve observations')
                case['observations'].append(values)
            case['status'] = 'passed'
            print('transient-runtime', directory.name, 'passed', flush=True)
        if digest(binary) != summary['binary_sha256']:
            raise RuntimeError('transient runtime MPI binary changed during validation')
