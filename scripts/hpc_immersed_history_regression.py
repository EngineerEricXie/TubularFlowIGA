#!/usr/bin/env python3
"""Check owned immersed velocity history and backward-Euler element parity."""
import argparse
import json
import math
import os
...
def main():
    ...
    binary = repo/'solvers/cpu/immersed_distributed_history_test'
    ...
    try:
        ...
        for ranks, split in cases:
            ...
            if result.returncode:
                raise RuntimeError(f'history/{ranks}: nonzero launcher exit')
            for rank in range(ranks):
                ...
                if report['returncode'] or report['timed_out'] or not report['resource']:
                    raise RuntimeError('failed or unmeasured history rank')
                case['rank_reports'].append(report)
                lines = [line for line in (rd/'stdout.log').read_text().splitlines()
                         if line.startswith('immersed_history_mpi ')]
                if len(lines) != 3 or any(not line.endswith(' passed') for line in lines):
                    raise RuntimeError('missing history MPI completion observation')
                expected_size = (1 if rank == 0 else 2) if split else ranks
                expected_rank = (0 if rank == 0 else rank-1) if split else rank
                for mode, line in enumerate(lines):
                    ...
                    if (any(not math.isfinite(value) for value in values.values())
                            ...
                            or values['mode'] != mode or values['element_error'] != 0
                            or values['inertia_effect'] <= 1e-6 or values['global_nodes'] != 112
                            or not 0 <= values['owned_nodes'] <= 112
                            or not 0 <= values['halo_nodes'] <= 112):
                        raise RuntimeError('invalid history MPI numerical observations')
                    case['observations'].append(values)
            case['status'] = 'passed'
            print('history', directory.name, 'passed', flush=True)
        if digest(binary) != summary['binary_sha256']:
            raise RuntimeError('history MPI binary changed during validation')
