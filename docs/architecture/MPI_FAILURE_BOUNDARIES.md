...
| F02 | mesh check／assembly smoke 的 result logging 與 resource summary 已 flush 並通過 84 個原生作業；CPU flow／runtime、configured／legacy transport 亦已補強，412 個作業及 310 個場比較通過；1D／graph／sequential 另通過 216 個作業與 660 個輸出比較，四個序列工具通過 48 個作業；CUDA 尚待補齊 | 接續 CUDA required logging；[耦合與工具 stdout 報告](../progress/HPC_01C_COUPLING_STDOUT_PROGRESS.md) 與 [CPU stdout 報告](../progress/HPC_01C_SOLVER_STDOUT_PROGRESS.md) 含 prefix／stage 對應與限制。區分可控制 stream failure 與 SIGPIPE／程序死亡 |
