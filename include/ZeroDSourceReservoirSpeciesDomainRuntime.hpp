#ifndef IGA_ZERO_D_SOURCE_RESERVOIR_SPECIES_DOMAIN_RUNTIME_HPP
#define IGA_ZERO_D_SOURCE_RESERVOIR_SPECIES_DOMAIN_RUNTIME_HPP
...
inline CouplingPort MakeZeroDSourceReservoirSpeciesPort(const std::string& domain_id,
	const std::set<std::string>& species_ids)
{
	if(species_ids.empty())throw std::invalid_argument("0D source species set is empty");
	auto port=MakeZeroDFlowPort(domain_id,ZeroDFlowRole::SourceReservoir);
...
class ZeroDSourceReservoirSpeciesDomainRuntime final : public CoupledDomainRuntime,
	public StagedFlowTransportDomainRuntime
{
public:
	ZeroDSourceReservoirSpeciesDomainRuntime(std::string domain_id,
		...
		std::map<std::string,double> pump_donor_concentration_mol_m3={},
		...
		:domain_id_(std::move(domain_id)),hydraulic_model_(std::move(hydraulic_model)),
		 ...
		 pump_donor_(std::move(pump_donor_concentration_mol_m3)),
		 committed_time_s_(initial_time_s)
	{
		if(domain_id_.empty()||!std::isfinite(initial_time_s)
			||hydraulic_model_.role!=ZeroDFlowRole::SourceReservoir
			||species_model_.port_ids!=std::set<std::string>{"graph","pump"})
			throw std::invalid_argument("0D source species domain configuration is invalid");
		...
		if(hydraulic_model_.source.prescribed_flow_m3_s>0.0)
			ValidateDonor(pump_donor_,"pump");
		else if(!pump_donor_.empty())
			throw std::invalid_argument("0D source non-inflow pump has donor concentration");
		ports_.push_back(MakeZeroDSourceReservoirSpeciesPort(domain_id_,
	...
	ZeroDFlowSpeciesCheckpointState CaptureCheckpointState()const
	{
		...
		if(committed_step_index_<0)
			throw std::runtime_error("0D source checkpoint requires an accepted step");
	...
	void RestoreCheckpointState(ZeroDFlowSpeciesCheckpointState value)
	{
		...
		if(committed_step_index_>=0||value.domain_id!=domain_id_
			...
			throw std::runtime_error("0D source checkpoint identity or fresh-state mismatch");
	...
	void BeginStep(const DomainStepContext& step)override
	{
		...
		if(committed_step_index_==std::numeric_limits<int>::max()
			...
			throw std::runtime_error("0D source species step differs from committed clock");
	...
	void SetPortInput(const std::string& port_id,const PortBoundaryData& input)override
	{
		...
		if(input.time_s!=step_.EndTime()||!input.mean_pressure_pa
			||input.outward_flow_m3_s||input.mean_normal_traction_pa
			||input.total_pressure_pa||!input.concentration.empty()
			||!input.outward_species_flux.empty()||pressure_input_)
			throw std::invalid_argument("0D source species requires one pressure-only input");
		pressure_input_=*input.mean_pressure_pa;
	}
	void SolveHydraulicTrial()override
	{
		RequirePhase(Phase::TrialReady,"solve hydraulic trial");
		if(!pressure_input_)throw std::runtime_error("0D source species is missing pressure input");
		hydraulic_trial_=EvaluateZeroDFlowTrial(hydraulic_model_,hydraulic_committed_,
			*pressure_input_,step_.dt_s,step_.start_time_s);
	...
	PortState GetHydraulicPortState(const std::string& port_id)const override
	{
		...
		if(phase_!=Phase::HydraulicSolved&&phase_!=Phase::TransportSolved
			...
			throw std::runtime_error("0D source hydraulic trial state is unavailable");
		...
		const std::map<std::string,double>& concentration)override
	{
		...
		if(time_s!=step_.EndTime()||*hydraulic_trial_->port.outward_flow_m3_s>=0.0
			||graph_donor_)
			throw std::invalid_argument("0D source graph donor is out of phase or direction");
		ValidateDonor(concentration,"graph");graph_donor_=concentration;
	}
	void SolveTransportTrial()override
	{
		RequirePhase(Phase::HydraulicSolved,"solve transport trial");
		if(*hydraulic_trial_->port.outward_flow_m3_s<0.0&&!graph_donor_)
			throw std::runtime_error("0D source graph inflow needs donor concentration");
	...
	PortState GetTransportPortState(const std::string& port_id)const override
	{
		...
		if(phase_!=Phase::HydraulicSolved
			||*hydraulic_trial_->port.outward_flow_m3_s<0.0)
			throw std::runtime_error("0D source outgoing donor state is unavailable");
	...
	{
		if(phase_!=Phase::TransportSolved&&phase_!=Phase::Prepared)
			throw std::runtime_error("0D source species accounting is unavailable");
		...
		for(auto& species:result){
			...
			if(node.empty())throw std::runtime_error("0D source graph amount is missing");
			node.key()="port";
	...
	void AbortStep()override
	{
		if(phase_==Phase::Committed)
			throw std::runtime_error("0D source abort requires an active step");
...
private:
	...
	struct Prepared{ZeroDFlowState hydraulic;ZeroDSpeciesReservoirState species;};
	void RequirePhase(Phase expected,const char* operation)const
	{
		if(phase_!=expected)
			throw std::runtime_error(std::string("0D source ")+operation+" is out of phase");
	}
	void RequirePort(const std::string& port_id)const
	{
		if(port_id!="port")throw std::invalid_argument("0D source species port is unknown");
	}
	std::string ModelIdentitySha256()const
	{
		return BuildZeroDFlowSpeciesModelIdentitySha256(hydraulic_model_,
			species_model_,pump_donor_);
	}
	void ValidateDonor(const std::map<std::string,double>& donor,const char* location)const
	{
		if(donor.size()!=species_model_.species_ids.size())
			throw std::invalid_argument(std::string("0D source ")+location+" donor set is incomplete");
		for(const auto& species:species_model_.species_ids){
			const auto found=donor.find(species);
			if(found==donor.end()||!(found->second>=0.0)||!std::isfinite(found->second))
				throw std::invalid_argument(std::string("0D source ")+location+" donor is invalid");
		}
	}
	ZeroDSourceReservoirSpeciesTrial CombinedTrial()const
	{
		const std::map<std::string,double> empty;
		return EvaluateZeroDSourceReservoirSpeciesTrial(hydraulic_model_,
			hydraulic_committed_,species_model_,species_committed_,*pressure_input_,
			graph_donor_?*graph_donor_:empty,pump_donor_,step_.dt_s,step_.start_time_s);
	}
	...
	{
		pressure_input_.reset();graph_donor_.reset();hydraulic_trial_.reset();
	...
	std::map<std::string,double> pump_donor_;
	...
	std::optional<double> pressure_input_;
