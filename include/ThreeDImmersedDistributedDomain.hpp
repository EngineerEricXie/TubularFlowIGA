#ifndef IGA_THREE_D_IMMERSED_DISTRIBUTED_DOMAIN_HPP
#define IGA_THREE_D_IMMERSED_DISTRIBUTED_DOMAIN_HPP

#include "ThreeDImmersedFlowDomain.hpp"
#include "../solvers/cpu/include/CollectiveFailure.hpp"
#include "../solvers/cpu/include/PetscReadArray.hpp"

namespace iga {

// Shared graph transactions. Lifecycle supplies the backend-specific trial
// boundary; all mutating methods are collective, while getters remain local.
template<class Runtime,class Lifecycle>
class ThreeDImmersedDistributedDomain : public CoupledDomainRuntime {
public:
	ThreeDImmersedDistributedDomain(const std::string& domain_id,Runtime& runtime,
		...
		: runtime_(runtime),communicator_(runtime.Communicator())
	{
		...
		Local("distributed immersed domain metadata",[&] {
			...
			Lifecycle::ValidateInitial(runtime_,initial_time_s);
			...
			Append(text,Lifecycle::Name()); Append(text,domain_id_); text << initial_time_s << ':' << ports_.size() << ':';
	...
	void BeginStep(const DomainStepContext& step) override
	{
		...
		Local("distributed immersed begin preflight",[&] {
			...
			Lifecycle::ValidateStep(runtime_,step);
	...
	void SolveTrial() override
	{
		...
		try {
			...
			Lifecycle::Begin(runtime_,step_);
	...
	void RollbackTrial() override
	{
		...
		Lifecycle::Abort(runtime_); RestoreControls();
