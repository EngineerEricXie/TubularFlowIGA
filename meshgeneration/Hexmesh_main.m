...
if ~isfinite(ratio_refine) || ratio_refine<0
    error('ratio_refine must be a non-negative finite value');
end
...
tip_label=1;
branch_label=-2;
minimum_scaled_jacobian=1.0e-3;
...
for i=1:ny
    for j=1:ny
        if trees{1}.dA(i,j)~=0
            ...
            segment_length=norm(Segment_Vector{i,j});
            diameter_scale=min(d(i),d(j));
            if ~isfinite(segment_length) || segment_length<=0
                error('Skeleton segment (%d,%d) has zero or invalid length',i,j);
            end
            if ~isfinite(diameter_scale) || diameter_scale<=0
                error('Skeleton segment (%d,%d) has zero or invalid diameter',i,j);
            end
            % Keep the axial length comparable to the local diameter.
            n_layer(i,j)=max(1,ceil(segment_length/(1.1*diameter_scale)));
            ...
            if(branch(i))
                n_layer(i,j)=max(n_layer(i,j),ceil(segment_length/(d(i)*ratio_refine)));
            end
            if(branch(j))
                n_layer(i,j)=max(n_layer(i,j),ceil(segment_length/(d(j)*ratio_refine)));
            end
        end
    end
end
...
if n_bif>0 && ratio_refine<=0
    error('ratio_refine must be positive when the skeleton contains bifurcations');
end
...
for index_bif=1:n_bif
    ...
    aij=acos(max(-1,min(1,dot(vi,vj)/norm(vi)/norm(vj))));
    aik=acos(max(-1,min(1,dot(vi,vk)/norm(vi)/norm(vk))));
    akj=acos(max(-1,min(1,dot(vk,vj)/norm(vk)/norm(vj))));
    ...
    spik=(ciki*kik2+cikk*kik1)/(kik1+kik2);
...
%         sec_vec_end=Segment_Vector{index_end_pt,id(index_end_pt)};
        ...
        angle_total=acos(max(-1,min(1,dot(ref_vec_start2end,ref_vec_end)/norm(ref_vec_start2end)/norm(ref_vec_end))));
...
%                     elseif dot(rotate_axis,[0 0 1])<0
        ...
        angle_start=acos(max(-1,min(1,dot(ref_temp2start,ref_vec_start)/norm(ref_vec_start)/norm(ref_temp2start))));
...
for index_bif=1:n_bif
    ...
    for i=1:num_bc
        point_index=startpt_b+botbc_index(i);
        candidate=PointAlign(AllPoint(startpt_j+rightbc_index(i),:),AllPoint(startpt_k+leftbc_index(i),:),AllPoint(point_index,:));
        [AllPoint,alpha]=MovePointSafely(AllPoint,AllElement,point_index,candidate,minimum_scaled_jacobian);
        if alpha<1
            fprintf('Limited bifurcation %d alignment point %d to alpha=%g\n',index_bif,point_index,alpha);
        end

        point_index=startpt_b+leftbc_index(i);
        candidate=PointAlign(AllPoint(startpt_i+leftbc_index(i),:),AllPoint(startpt_j+leftbc_index(i),:),AllPoint(point_index,:));
        [AllPoint,alpha]=MovePointSafely(AllPoint,AllElement,point_index,candidate,minimum_scaled_jacobian);
        if alpha<1
            fprintf('Limited bifurcation %d alignment point %d to alpha=%g\n',index_bif,point_index,alpha);
        end

        point_index=startpt_b+rightbc_index(i);
        candidate=PointAlign(AllPoint(startpt_i+rightbc_index(i),:),AllPoint(startpt_k+rightbc_index(i),:),AllPoint(point_index,:));
        [AllPoint,alpha]=MovePointSafely(AllPoint,AllElement,point_index,candidate,minimum_scaled_jacobian);
        if alpha<1
            fprintf('Limited bifurcation %d alignment point %d to alpha=%g\n',index_bif,point_index,alpha);
        end
    ...
    for i=1:2
        plane_a=AllPoint(startpt_b+extra_pt(i,2),:);
        plane_b=AllPoint(startpt_b+extra_pt(i,3),:);
        plane_c=AllPoint(startpt_b+extra_pt(i,4),:);
        point_indices=[startpt_b,startpt_i,startpt_j,startpt_k]+extra_pt(i,1);
        for point_index=point_indices
            candidate=Projection(plane_a,plane_b,plane_c,AllPoint(point_index,:));
            [AllPoint,alpha]=MovePointSafely(AllPoint,AllElement,point_index,candidate,minimum_scaled_jacobian);
            if alpha<1
                fprintf('Limited bifurcation %d projection point %d to alpha=%g\n',index_bif,point_index,alpha);
            end
        end
    end
end

[minimum_det,minimum_scaled,bad_elements,first_bad_element]=HexMeshQuality(AllPoint,AllElement);
fprintf('Control mesh quality: min detJ=%g min scaled J=%g bad elements=%d\n',minimum_det,minimum_scaled,bad_elements);
if bad_elements>0
    error('Generated mesh has %d invalid elements; first invalid element is %d',bad_elements,first_bad_element-1);
end
if minimum_scaled<minimum_scaled_jacobian
    error('Generated mesh minimum scaled Jacobian %g is below the %g quality floor',minimum_scaled,minimum_scaled_jacobian);
end
