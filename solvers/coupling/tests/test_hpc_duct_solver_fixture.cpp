...
int main(int argc, char** argv)
{
	try {
		...
		Require(argc==5||argc==6,
			"usage: hpc_duct_solver_fixture ROOT TRANSVERSE AXIAL RANKS [STEPS] | --validate OUTPUT");
		...
		const int steps = argc==6 ? positive(argv[5],1000000,"step count") : 2;
		...
		std::cout << "duct_fixture transverse=" << transverse << " axial=" << axial
			...
			<< " nodes=" << (transverse+3)*(transverse+3)*(axial+3) << " steps=" << steps
			<< " ranks=" << ranks << '\n';
