...
RawSurfaceTriangle Face(std::int64_t a, std::int64_t b, std::int64_t c, int label)
{
...
int main()
{
	try {
		...
		const auto affine_state=ImmersedGlobalFlowState(0.,4,expanded.layout,Affine(expanded.layout,expanded.grid),{0.},false,0.);const auto affine=MovingImmersedFlowSnapshot::Build(*expanded.geometry,expanded.layout,affine_state,Request(expanded),options);Check(!affine.Points().empty(),"affine snapshot is empty");
		...
		constexpr double omega=1.75;const auto rotation_state=ImmersedGlobalFlowState(0.,4,expanded.layout,Rotation(expanded.layout,expanded.grid,omega),{0.},false,0.);const auto request=Request(expanded);const auto snapshot=MovingImmersedFlowSnapshot::Build(*expanded.geometry,expanded.layout,rotation_state,request,options);const auto repeated=MovingImmersedFlowSnapshot::Build(*expanded.geometry,expanded.layout,rotation_state,request,options);const auto& metrics=snapshot.Metrics();Check(snapshot.ContentHashSha256()==repeated.ContentHashSha256()&&snapshot.SnapshotIdentitySha256()==repeated.SnapshotIdentitySha256(),"snapshot hashes are not deterministic");
