...
// Steady MPI Newton solve with owned committed/prepared states. Public MPI
...
class ImmersedStaticDistributedRuntime {
public:
		...
		: communicator_(communicator)
	{
		...
		try {
			...
			Check("distributed static KSP prefix",KSPSetOptionsPrefix(solver_,"immersed_static_"));
			Check("distributed static KSP options",KSPSetFromOptions(solver_));
		} catch (...) { Release(); throw; }
	}
	~ImmersedStaticDistributedRuntime() { Release(); }
	...
	void Close()
	{
		Release(); op_->Close(); cleanup_.Check(communicator_,"distributed static close");
	}
private:
	const ImmersedStaticFlowOptions& Options() const noexcept { return op_->Options(); }
	...
	std::unique_ptr<ImmersedStaticDistributedOperator> op_;
