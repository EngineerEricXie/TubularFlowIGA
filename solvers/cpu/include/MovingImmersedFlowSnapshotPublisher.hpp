...
private:
	...
	static void WriteVtu(const std::filesystem::path& path, const MovingImmersedFlowSnapshot& s, const MovingImmersedFlowSnapshotPublicationIdentity& id)
	{
		...
		auto fields=[&](const char* type,const char* name,unsigned n,auto getter){std::ostringstream values;values.exceptions(std::ios::badbit | std::ios::failbit);values<<std::setprecision(17);for(const auto& p:s.Points())getter(values,p);Data(out,type,name,n,values.str());};
		...
		out<<"      </PointData>\n      <Points>\n"; {std::ostringstream values;values.exceptions(std::ios::badbit | std::ios::failbit);values<<std::setprecision(17);for(const auto&p:s.Points())for(double x:p.physical_m)values<<x<<' ';Data(out,"Float64","points",3,values.str());} out<<"      </Points>\n      <Cells>\n";
		{std::ostringstream v;v.exceptions(std::ios::badbit | std::ios::failbit);for(std::size_t i=0;i<s.Points().size();++i)v<<i<<' ';Data(out,"Int64","connectivity",1,v.str());} {std::ostringstream v;v.exceptions(std::ios::badbit | std::ios::failbit);for(std::size_t i=0;i<s.Points().size();++i)v<<(i+1)<<' ';Data(out,"Int64","offsets",1,v.str());} {std::ostringstream v;v.exceptions(std::ios::badbit | std::ios::failbit);for(std::size_t i=0;i<s.Points().size();++i)v<<1<<' ';Data(out,"UInt8","types",1,v.str());}
		out<<"      </Cells>\n    </Piece>\n  </UnstructuredGrid>\n</VTKFile>\n"; out.close();if(!out)throw std::runtime_error("cannot write moving snapshot VTU");
	}
	static std::string Number(double value) { if(!std::isfinite(value))throw std::invalid_argument("moving snapshot JSON value is nonfinite");std::ostringstream out;out.exceptions(std::ios::badbit | std::ios::failbit);out<<std::setprecision(17)<<value;return out.str(); }
	...
	static std::string ReadAll(const std::filesystem::path&p){std::ifstream in(p,std::ios::binary);if(!in)throw std::runtime_error("moving snapshot epoch file is missing");return ReadCheckedText(in);}
	...
	static std::string Nonce(){std::ostringstream out;out.exceptions(std::ios::badbit | std::ios::failbit);out<<std::chrono::steady_clock::now().time_since_epoch().count()<<'.'<<std::random_device{}();return out.str();}
