...
{
	PressureFlowExecutionSynchronization result;
	result.outcome = [communicator](const char* stage, std::exception_ptr error) {
			CollectiveLocalStage(communicator, stage, [&] {
				if (error) std::rethrow_exception(error);
			});
		};
	result.all_converged = [communicator](bool converged) {
