...
private:
	...
	static std::vector<std::int32_t> FaceConnectivity(const CartesianDomainClassification&d,const CutCellGhostPenaltyFace&f){return CubicCartesianSplineFaceConnectivity(d,f.minus_cell,f.plus_cell);}
	...
	void Precompute(const CartesianDomainClassification&d,std::size_t fi){const auto&f=faces_[fi];const double ku=NormalizedCoefficient(options_.gamma_u,f.h_normal_m,5),kp=NormalizedCoefficient(options_.gamma_p,f.h_normal_m,7);const auto nodes=FaceConnectivity(d,f);for(const auto&q:CubicCartesianSplineFaceTangentialQuadrature())for(double z:CubicCartesianSplineFaceNormalDerivativeJump(d.Background().MaterializeElement(f.minus_cell),d.Background().MaterializeElement(f.plus_cell),f.axis,3,q.tangential_0,q.tangential_1,f.h_normal_m,nodes)){if(!std::isfinite(z))throw std::overflow_error("ghost penalty trace diagnostic is not finite");diagnostics_.maximum_abs_trace_jump=std::max(diagnostics_.maximum_abs_trace_jump,std::abs(z));}diagnostics_.maximum_abs_coefficient=std::max(diagnostics_.maximum_abs_coefficient,std::max(ku,kp));}
