...
struct ImmersedTransientFlowOptions {
	...
	// Explicit pressure-port labels only. Empty preserves the legacy model.
	std::map<int,double> pressure_port_backflow_beta;
            ...
            [&](const PreparedVolume& input, const ElementSystems& systems, std::size_t) {
                ...
                if(systems.trace) {
                    ...
                    auto ports_timer=detail.Time("ports");
	                    for(std::size_t p=0;p<options_.ports.size();++p) if(RuleHasLabel(rule,options_.ports[p].boundary_label)) ScatterPort(input.cell,input.element,input.nodal,rule,p,full);
                }
            });
...
private:
	...
	void ConfigurePorts() { std::vector<int> labels; for(const auto& p:options_.ports){ValidateImmersedFlowPortDefinition(p); labels.push_back(p.boundary_label);} std::sort(labels.begin(),labels.end()); if(std::adjacent_find(labels.begin(),labels.end())!=labels.end()) throw std::invalid_argument("immersed transient port labels must be unique"); for(std::size_t i=0;i<options_.ports.size();++i){for(std::size_t j=0;j<i;++j)if(options_.ports[i].id==options_.ports[j].id)throw std::invalid_argument("immersed transient port ids must be unique"); if(std::binary_search(options_.wall_labels.begin(),options_.wall_labels.end(),options_.ports[i].boundary_label)) throw std::invalid_argument("immersed transient wall and port labels overlap"); diagnostics_.ports.push_back({options_.ports[i].id,options_.ports[i].boundary_label,options_.ports[i].control_mode,options_.ports[i].value});}
		for (const auto& entry : options_.pressure_port_backflow_beta) {
			if (!(entry.second >= 0.0) || !std::isfinite(entry.second))
				throw std::invalid_argument("immersed pressure-port backflow beta must be finite and nonnegative");
			const auto found = std::find_if(options_.ports.begin(), options_.ports.end(), [&](const auto& port) {
				return port.boundary_label == entry.first && IsImmersedFlowPressureLike(port.control_mode);
			});
			if (found == options_.ports.end())
				throw std::invalid_argument("immersed backflow label must select a pressure-like port");
		}
	...
	void ScatterPort(std::uint64_t cell,const Element&e,const std::vector<std::array<double,4>>&n,const SurfaceQuadratureRule&r,std::size_t i,bool full)
	{
		const auto& p=options_.ports[i];
		const auto x=BuildImmersedFlowPortElement(e,r,p.boundary_label,p.control_mode,p.value,n);
		std::vector<PetscInt> rows;
		for(auto id:e.connectivity)for(int q=0;q<4;++q)rows.push_back(Dof(id,q));
		if(IsImmersedFlowPressureLike(p.control_mode)){
			Check(VecSetValues(rhs_,rows.size(),rows.data(),x.negative_residual.data(),ADD_VALUES),"VecSetValues port");
			const auto selected=options_.pressure_port_backflow_beta.find(p.boundary_label);
			if(selected!=options_.pressure_port_backflow_beta.end()&&selected->second>0.0){
				const auto& provenance=surface_.UsableProvenance(domain_,cell);
				if(provenance.size()!=r.Points().size())throw std::logic_error("immersed pressure-port provenance size differs from quadrature");
				std::vector<std::array<double,3>> material_velocity(provenance.size());
				for(std::size_t point=0;point<provenance.size();++point)
					if(r.Points()[point].boundary_id==p.boundary_label)material_velocity[point]=MaterialVelocityAt(provenance[point]);
				Scatter(e.connectivity,BuildImmersedPressurePortBackflowElement(e,r,p.boundary_label,n,
					material_velocity,options_.parameters.density,selected->second,
					full?NavierStokesAssemblyRequest::ResidualAndJacobian:NavierStokesAssemblyRequest::ResidualOnly));
			}
			return;
		}
		const auto scalar=static_cast<PetscInt>(layout_.ControllerRow(static_cast<std::uint64_t>(p.boundary_label)));
		const double lambda=Value(state_,scalar);
		for(std::size_t k=0;k<rows.size();++k){const double c=PetscRealPart(x.flow_coefficient[k]);if(c){
			if(full){Check(MatSetValue(jacobian_,rows[k],scalar,c,ADD_VALUES),"MatSetValue port col");Check(MatSetValue(jacobian_,scalar,rows[k],c,ADD_VALUES),"MatSetValue port row");}
			Check(VecSetValue(rhs_,rows[k],-lambda*c,ADD_VALUES),"VecSetValue port");
		}}
	}
	...
	void AppendBackflowOptions(Sha256& h) const
	{
		if (options_.pressure_port_backflow_beta.empty()) return;
		immersed_transient_detail::AppendString(h,"ImmersedPressurePortBackflow/v1");
		h.AppendLittleEndian64(options_.pressure_port_backflow_beta.size());
		for (const auto& entry : options_.pressure_port_backflow_beta) {
			h.AppendLittleEndian32(static_cast<std::uint32_t>(entry.first));
			h.AppendNormalizedDouble(entry.second);
		}
	}
