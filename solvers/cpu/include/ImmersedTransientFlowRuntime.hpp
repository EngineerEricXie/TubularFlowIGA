...
public:
	...
	bool SolveTrial()
	{
		...
		try { for(PetscInt it=0;it<options_.nonlinear_maximum_iterations;++it) { Assemble(); PetscReal norm=0; Check(VecNorm(rhs_,NORM_2,&norm),"VecNorm residual"); if(!std::isfinite(norm)) throw std::runtime_error("immersed transient nonlinear residual is not finite"); if(initial<0) { initial=norm; initial_blocks=ResidualBlockNorms(Copy(rhs_)); } diagnostics_.residual_norm=norm; diagnostics_.nonlinear_iterations=it;
			...
			Check(VecCopy(rhs_,action_input_),"VecCopy rhs"); Check(KSPSetOperators(ksp_,jacobian_,jacobian_),"KSPSetOperators"); const auto begin=std::chrono::steady_clock::now(); solver_options_->Call("immersed transient solve", [&] { return SolveProfiledKsp(ksp_,rhs_,update_); }); solver_options_->RecordUsed(); diagnostics_.last_linear_solve_seconds=std::chrono::duration<double>(std::chrono::steady_clock::now()-begin).count();
